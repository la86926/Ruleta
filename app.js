(()=>{
  ...
  function resetSpinState(){
    ...
    wheelWrap.classList.remove('repeat-mode','is-removing','is-closing','just-settled');
  }
  ...
  function drawWheel(scene={}){
    fitCanvas();
    const renderItems=scene.items||items;
    const size=canvas.width,center=size/2,radius=size*.485;
    ...
    for(let i=0;i<renderItems.length;i++){
      ...
      drawSegment(item,start,end,center,radius,size,fontSize,i%labelStep===0,opacity,glow);
    }
  ...
  async function presentWinner(winner){
    ...
    if(!REDUCED_MOTION){
      await tween(720,t=>{const wave=Math.sin(Math.PI*t);drawWheel({targetId:winner.id,targetOpacity:1,glow:.18+.82*wave})},token);
    }
    if(token!==animationToken)return;
    if(autoHideToggle.checked&&items.some(i=>i.id===winner.id)){
      if(!REDUCED_MOTION)await wait(420,token);
      if(token!==animationToken)return;
  ...
  async function animateRemoveInternal(id,announce,token,fromAuto){
    ...
    const fadeOk=await tween(760,t=>{const ease=t*t*(3-2*t);drawWheel({items:oldItems,targetId:id,targetOpacity:1-ease,glow:(1-ease)*.55})},token);
    if(!fadeOk||token!==animationToken)return;

    wheelWrap.classList.remove('is-removing');wheelWrap.classList.add('is-closing');
    const closeOk=await tween(820,t=>drawWheel({items:oldItems,closing:true,removedIndex,progress:t}),token);
    if(!closeOk||token!==animationToken)return;

    items=newItems;hiddenItems.push(removed);syncTextarea();resultCard.classList.remove('is-hiding');wheelWrap.classList.remove('is-closing');wheelWrap.classList.add('just-settled');setTimeout(()=>wheelWrap.classList.remove('just-settled'),620);
  ...
  function persist(){
    try{localStorage.setItem('ruleta-state-v3',JSON.stringify({title:titleInput.value,items:items.map(i=>i.label),hidden:hiddenItems.map(i=>i.label),history,autoHide:autoHideToggle.checked}))}catch(_){}
  }

  function restorePersisted(){
    try{
      const raw=localStorage.getItem('ruleta-state-v3')||localStorage.getItem('ruleta-state-v2')||localStorage.getItem('ruleta-state-v1');if(!raw)return;
      const saved=JSON.parse(raw);
      if(Array.isArray(saved.items)&&saved.items.length){items=saved.items.slice(0,250).map(makeItem);itemsInput.value=items.map(i=>i.label).join('\n')}
      if(Array.isArray(saved.hidden))hiddenItems=saved.hidden.slice(0,250).map(makeItem);
